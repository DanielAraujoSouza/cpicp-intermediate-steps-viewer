...
class CloudViewer {
  ...
  constructor(containerId) {
    ...
    this.axisScene.add( new THREE.ArrowHelper( new THREE.Vector3( 1,0,0 ), arrowPos, 100, 0xff0000, 20, 10 ) );
    this.axisScene.add( new THREE.ArrowHelper( new THREE.Vector3( 0,1,0 ), arrowPos, 100, 0x00ff00, 20, 10 ) );
    this.axisScene.add( new THREE.ArrowHelper( new THREE.Vector3( 0,0,1 ), arrowPos, 100, 0x0000ff, 20, 10 ) );
  ...
  removeCloudByLabel(cloudLbl) {
    const childId = this.cloudGroup.children.findIndex(e => e.name === cloudLbl);
  ...
  #createAxisCanvas() {
    ...
    canvas.classList.add('mx-4', 'my-2', 'position-absolute', 'bottom-0','end-0');
  ...
  #createMenu() {
    ...
    {     
      ...
      // Cloud Menu
      {
        ...
        cloudRow.classList.add('bg-light', 'd-flex', 'align-items-center', 'px-3');
  ...
  #dowloadCloudById(cloudId) {
    ...
    if (cloud) {
      ...
      const cloudData = URL.createObjectURL(new Blob([JSON.stringify(cloudObj, null, 2)], {
        type: "text/plain"
      }));
  ...
  #getGroupBoundingSphere() {
    ...
    return bbox.getBoundingSphere(new THREE.Sphere(bbox.getCenter(new THREE.Vector3())));
  }
  ...
  #includeMenuOption(label, id, color, size) {
    ...
    row.classList.add('d-flex', 'flex-row', 'w-100', 'align-items-center', 'p-2','list-group-item-action');
    ...
    // Download JSON Cloud
    {
      ...
      dowloadCloudBtnIcon.classList.add('fas', 'fa-file-download', 'link-secondary');
    ...
    // Hidden Button
    {
      ...
      hideBtn.setAttribute('title', 'Ocultar Nuvem');
  ...
  #updateCloudColor(cloudId, newColor) {
    this.cloudGroup.children.filter(e => e.id === +cloudId).forEach(cloud => {
